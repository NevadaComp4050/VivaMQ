...
import { AlertCircle, Loader2, Download, FileQuestion, RefreshCw, Plus, Trash2 } from "lucide-react"
...
export default function ViewRubricPage({
  ...
  const handleCriterionChange = (index: number, field: string, value: string | number) => {
    ...
    if (field === "name" || field === "marks") {
      ...
      updatedCriteria[index] = {
        ...updatedCriteria[index],
        descriptors: { ...updatedCriteria[index].descriptors, [field]: value }
      }
    }
    setRubric({
      ...rubric,
  ...
  const handleRemoveCriterion = (index: number) => {
    ...
    const updatedCriteria = rubric.rubricData.criteria.filter((_, i) => i !== index)
    setRubric({
      ...rubric,
      rubricData: { ...rubric.rubricData, criteria: updatedCriteria }
    })
    ...
    if (loading) {
      return (
        <div className="flex flex-col items-center justify-center h-64">
          ...
          <p className="text-sm text-muted-foreground">Please wait while we fetch the rubric data.</p>
        </div>
      )
    }
    ...
    if (!rubric) {
      return (
        <div className="flex flex-col items-center justify-center h-64">
          ...
          <p className="text-sm text-muted-foreground mb-4">The requested rubric could not be found or may have been deleted.</p>
          <Button onClick={() => router.push("/dashboard/rubrics")}>View All Rubrics</Button>
        </div>
      )
    }
    ...
    if (rubric.status === "PENDING") {
      return (
        <div className="flex flex-col items-center justify-center h-64">
          ...
          <p className="text-sm text-muted-foreground">This may take a few moments. We'll refresh automatically when it's ready.</p>
        </div>
      )
    }
    ...
    if (!rubric.rubricData) {
      return (
        <div className="flex flex-col items-center justify-center h-64">
          ...
          <p className="text-sm text-muted-foreground mb-4">This rubric doesn't have any data yet. It may still be processing or there might have been an error during generation.</p>
          <Button onClick={fetchRubric}>
            <RefreshCw className="mr-2 h-4 w-4" />
            Check Again
          </Button>
              ...
              <TableRow key={index}>
                ...
                <TableCell>
                  <Textarea
                    ...
                    onChange={(e) => handleCriterionChange(index, "F", e.target.value)}
                    className="min-h-[100px]"
                  />
                </TableCell>
                <TableCell>
                  <Textarea
                    ...
                    onChange={(e) => handleCriterionChange(index, "P", e.target.value)}
                    className="min-h-[100px]"
                  />
                </TableCell>
                <TableCell>
                  <Textarea
                    ...
                    onChange={(e) => handleCriterionChange(index, "C", e.target.value)}
                    className="min-h-[100px]"
                  />
                </TableCell>
                <TableCell>
                  <Textarea
                    ...
                    onChange={(e) => handleCriterionChange(index, "D", e.target.value)}
                    className="min-h-[100px]"
                  />
                </TableCell>
                <TableCell>
                  <Textarea
                    ...
                    onChange={(e) => handleCriterionChange(index, "HD", e.target.value)}
                    className="min-h-[100px]"
                  />
                </TableCell>
                <TableCell>
                  <Input
                    ...
                    onChange={(e) => handleCriterionChange(index, "marks", parseInt(e.target.value))}
                    className="w-16"
                  />
                </TableCell>
                <TableCell>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              {rubric?.status === "PENDING" ? "Generating rubric..." : "View and edit rubric details"}
            </CardDescription>
          </div>
          <div className="flex space-x-2">
            <Button onClick={() => router.push("/dashboard/rubrics")} variant="outline">
              Back
            </Button>
            <DropdownMenu>
